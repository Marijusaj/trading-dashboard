...
export type SignalStatus = "setup" | "breakout" | "confirmed" | "invalidated";

export function getSignalStatus(price: number, signal: TradeSignal): SignalStatus {
  if (price <= signal.invalidateLevel) return "invalidated";
  if (price >= signal.confirmLevel) return "confirmed";
  if (price >= signal.breakoutLevel) return "breakout";
  return "setup";
}
...
> = {
  ...
  breakout: {
    ...
    description: "Price above $0.32! Add size on daily close confirmation",
  },
  confirmed: {
    label: "CONFIRMED",
