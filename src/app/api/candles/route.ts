...
export async function GET(req: NextRequest) {
  ...
  // ── Source 2: CryptoCompare (proper daily candles, reliable on Vercel)
  // Prioritized over CoinGecko OHLC because CoinGecko returns 4-day aggregates
  // for days=90, which hides intraday moves and makes charts look stale.
  try {
    const isPairBtc = symbol.endsWith("BTC") && symbol !== "BTC";
    const fsym = symbol
      .replace(/USDT$/i, "")
      ...
      .replace(/BTC$/i, "")
      .toUpperCase() || "BTC";
    const tsym = isPairBtc ? "BTC" : "USD";
    const ccLimit = Math.min(parseInt(limit), 200);
    ...
    if (res.ok) {
      ...
      if (json.Data?.Data && Array.isArray(json.Data.Data) && json.Data.Data.length > 0) {
        const candles = json.Data.Data
          .filter((d: { open: number; close: number }) => d.open > 0 && d.close > 0)
          .map((d: { time: number; open: number; high: number; low: number; close: number; volumeto: number }) => ({
            time: d.time,
            open: d.open,
            high: d.high,
            low: d.low,
    ...
    // CryptoCompare failed — fall through to CoinGecko
  }

  // ── Source 3: CoinGecko OHLC (fallback — note: returns 4-day candles for days>30)
  const { geckoId, vsCurrency } = parseGeckoId(symbol);
  const days = interval === "1d" ? 30 : interval === "4h" ? 30 : 7; // days<=30 = daily/hourly, not 4-day
  ...
  // ── Source 4: CoinGecko market_chart (synthesize candles from daily prices)
  try {
    const url = `https://api.coingecko.com/api/v3/coins/${geckoId}/market_chart?vs_currency=${vsCurrency}&days=90&interval=daily`;
    const res = await fetch(url, { cache: "no-store" });
